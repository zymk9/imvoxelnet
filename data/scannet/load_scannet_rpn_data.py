# Further modified to load the ScanNet data used in NeRF-RPN.
# Since we filter out some objects in the NeRF version, we need to
# read exclusion lists and filter out the corresponding objects here.
...
def find_minimum_bounding_box(vertices):
    ...
    center = np.concatenate([center, [(min_z + max_z) / 2.0]])
...
def extract_bbox(mesh_vertices, object_id_to_segs, object_id_to_label_id,
                 instance_ids, obj_ids_to_keep):
    """Extract bbox from mesh vertices and instance ids.
        Modified to include the z-axis rotation and exclude certain objects.
    """
    # num_instances = len(np.unique(list(object_id_to_segs.keys())))

    objs_to_keep = np.unique(list(obj_ids_to_keep))  # results are sorted
    for id in objs_to_keep:
        assert id in object_id_to_segs
    num_instances = len(objs_to_keep)

    instance_bboxes = np.zeros((num_instances, 8))

    for idx, obj_id in enumerate(objs_to_keep):
        ...
        bbox = find_minimum_bounding_box(obj_pc)
        bbox = np.concatenate([bbox, np.array([label_id])])

        # NOTE: this assumes obj_id is in 1,2,3,.,,,.NUM_INSTANCES
        instance_bboxes[idx, :] = bbox
...
def export(mesh_file,
    ...
    Args:
        ...
        filter_file (str): Path to the filter file for loading NeRF-RPN ScanNet.
    ...
    # Objects used NeRF-RPN version ScanNet, with obj ids indexed from 0
    obj_ids_to_keep = np.load(filter_file)
    obj_ids_to_keep = obj_ids_to_keep + 1  # convert to obj ids indexed from 1
    obj_ids_to_keep = set(obj_ids_to_keep)
    ...
    if not test_mode:
        ...
        unaligned_bboxes = extract_bbox(mesh_vertices, object_id_to_segs,
                                        object_id_to_label_id, instance_ids, obj_ids_to_keep)
