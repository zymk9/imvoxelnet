# Modified further to load NeRF-RPN ScanNet data.
...
import numpy as np
import os
from load_scannet_rpn_data import export
...
def export_one_scan(scan_name,
    ...
    # Object ids to keep
    filter_file = osp.join(filter_dir, scan_name + '.npy')
    ...
    for scan_name in scan_names:
        ...
        try:
            export_one_scan(scan_name, output_filename_prefix, max_num_point,
                            label_map_file, scannet_dir, filter_dir, test_mode)
        except Exception:
...
def main():
    ...
    parser.add_argument(
        '--filter_dir',
        help='The path to the folder holding the object filter files, one for each scene.'
    )
